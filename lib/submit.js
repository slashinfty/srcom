  ...
  exe: async () => {
    ...
    const inquiry = async gameId => {
      queries.get('game').returnPoss(gameId).then(possible => {
        ...
        if (possible.categories[chosenCategory - 1].variables.length > 0) {
          possible.categories[chosenCategory - 1].variables.forEach(variable => {
            const subString = queries.get('stringify').returnString(variable.vals);
            console.log(chalk.green('\nChoose a ' + variable.name.toLowerCase() + ':\n' + subString));
            const chosenVariable = rl.question(chalk.bgGreen.black(variable.name + ':') + ' ');
            if (variable.vals[chosenVariable - 1] === undefined) {
              console.log(chalk.red('Not a valid selection.'));
              return;
            }
            submission.run.variables = {
              [variable.id]: {
                "type": "pre-defined",
                "value": variable.vals[chosenVariable - 1].id
              }
            }
          })
        }
