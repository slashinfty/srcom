    ...
    const inquiry = async gameId => {
      queries.get('game').returnPoss(gameId).then(possible => {
        ...
        possible.times.forEach(t => {
          ...
          submission.run.times[t] = inputTime;
        });
        console.log(chalk.yellow('Leave date blank for today\'s date.'));
        let d = new Date();
        submission.run.date = rl.question(chalk.bgGreen.black('Date (YYYY-MM-DD):') + ' ', {defaultInput: d.getFullYear() + '-' + (d.getMonth() + 1) + '-' + d.getDate()});
        ...
        submission.run.comments = rl.question(chalk.bgGreen.black('Comments:') + ' ');
        //TODO submission
      });
    }
